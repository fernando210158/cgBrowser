...
def aggFiles():
    if dFolder.get() == '' or tFolder.get() == '' or rFolder.get() == '':
        messagebox.showerror('HS4CG Diag', 'Set Folder Paths')
        pass
    else:
        sessInDat, sessInTrc = aggreg(dFolder.get(), tFolder.get(), rFolder.get())
        messagebox.showinfo(
            'Aggregate', f'# of SessionId in dat files: {sessInDat}, # of SessionId in trc files: {sessInTrc}')
    ...
    if rFolder.get() == '':
        ...
        def handle_left_click(event):
            ...
            if prSess.empty:
                ...
                prSess.sort_values('pWStartTime', axis=0, ascending=True, inplace=True)
            ...
            def handle_left_click1(event):
                ...
                #dropCol = ['Unnamed: 0', 'flow']
                #flSess.drop(dropCol, axis=1, inplace=True)
                subFrame1 = Toplevel()
